...
mod sha2_sanity {
    ...
    fn sha256_ref(msg: &[u8]) -> [u8; 32] {
        ...
        for (i, &w) in h.iter().enumerate() {
            out[i * 4..][..4].copy_from_slice(&w.to_be_bytes());
        }
        out
    }
    ...
    fn arithmetic_sanity() {
        ...
        let manual_rotr6 = x.rotate_right(6);
        assert_eq!(rotr6, manual_rotr6, "rotate_right(6) sanity check");
        // wrapping_add sanity
